...
Skills work correctly when they produce the expected output and route to the right task every time. Use [LangSmith evals](/langsmith/evaluation) to verify both.

<Note>
Evals require setup in LangSmith outside the Fleet UI. See [LangSmith evaluation](/langsmith/evaluation) to get started.
</Note>

### Evaluate output quality

Create a dataset of input prompts and expected outputs, define an evaluator (LLM-as-judge or a custom scoring function), and run experiments to track quality over time. Set up online evals to run automatically against live production traces.

### Verify skill routing

To confirm that a given intent routes to the correct skill, create a dataset of intents and expected skill names. Write an evaluator that checks whether the agent's chosen skill matches the expected one. Re-run the experiment whenever skills change to catch false negatives (the right skill does not fire) and false positives (the wrong skill fires).

### Observe full runs

[LangSmith tracing](/langsmith/observability) captures the full execution of every Fleet agent run, including tool calls, skill loads, and outputs. Inspect a trace to confirm the correct skill was used and the correct tool was called.

To automate this as a recurring check, create a LangSmith eval dataset with test queries and expected behaviors, write evaluators to assert correctness, and schedule the experiment.
