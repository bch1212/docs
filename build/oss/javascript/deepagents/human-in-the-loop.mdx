...
![Agents use interrup() to pause execution and surface a payload to a human for review; it also saves graph state to durable storage. When approved, the agent loads the graph state and continues.](/oss/images/deepagents/production/hitl.png)
